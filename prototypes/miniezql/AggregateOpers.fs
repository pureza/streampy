...
let makeLast getField (uid, prio, parents, context) =
  let eval = fun (op:Operator, inputs) ->
               ...
               match added with
               | None -> Nothing
               | Some ev -> let ev' = if op.Value <> VNull
                                        then match ev, op.Value with
                                             | VRecord fields1, VRecord fields2 -> VRecord (Map.merge (fun o n -> if n = VNull then o else n) fields2 fields1)
                                             | VNull, _ -> op.Value
                                             | _ -> ev
                                        else ev
                            setValueAndGetChanges op ev'
