...
let makeEvaluator expr kenv (uid, prio, (parents:Operator list), context) =
  ...
  let initialContents =
    if parents.IsEmpty
      then let primitives = Map.of_list [ for f in Primitives.primitiveFunctions -> (f.Name, VPrimitive f.Eval) ]
           let env = Map.union (Map.of_list [ for p in parents -> (p.Uid, p.Value) ]) primitives
           eval env expr
      else VNull
    ...
    Operator.Build(uid, prio,
      (fun (op:Operator, inputs) ->
           ...
           | VClosure _ as closure ->
               ...
               let curried = List.fold (fun acc param -> FuncCall (acc, param)) (Id (Identifier "$closure")) param's
               let result = eval (env.Add("$closure", closure)) curried
  ...
  let operEval = fun ((op:Operator), inputs) ->
                   ...
                   | _::listenerInputs ->
                       ...
                       let result = List.fold (fun value parentIdx ->
                                                 ...
                                                 let primitives = [ for f in Primitives.primitiveFunctions -> (f.Name, VPrimitive f.Eval) ]
                                                 let env = Map.of_list (["$curr", value] @ primitives)
