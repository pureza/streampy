...
and value =
    ...
    override self.ToString() =
      ...
      | VFloat f -> f.ToString()
      | VString s -> s
      | VSymbol sym -> sprintf ":%s" sym
      | VRecord m -> (sprintf "{ %s }" (Map.fold_left (fun acc k v -> acc + (sprintf " %O = %O," k v)) "" m))
      | VDict m -> (sprintf "{ %s }" (Map.fold_left (fun acc k v -> acc + (sprintf " %O = %O," k v)) "" m))
      | VTuple vs -> sprintf "(%s)" (List.reduce (fun acc v -> acc + sprintf ", %O" v) (List.map string vs))
      | VClosure _ | VClosureSpecial _ -> "<fun>"
      | VVariant (label, metadata) -> sprintf "%s (%O)" label metadata
      | VWindow values -> sprintf "[%s]" (List.fold (fun acc v -> sprintf "%s %O" acc v) "" values)
      | VPrimitive _ -> "primitive function"
      | VNull -> "(null)"
...
and PrimEvaluator = (Context -> expr -> value) -> Context -> expr -> value
