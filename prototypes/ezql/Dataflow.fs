...
let rec dataflow (env:NodeContext, types:TypeContext, (graph:DataflowGraph), roots) = function
  | Def (Identifier name, exp) ->
      let deps, g', expr', _ = dataflowE env types graph exp
      ...
      env.Add(name, n).Add(n.Uid, n), types.Add(n.Uid, n.Type), g'', roots'
  | Expr expr ->
      let deps, g', expr', _ = dataflowE env types graph expr
      env, types, g', roots
...
and dataflowE (env:NodeContext) types (graph:DataflowGraph) expr =   
  ...
  | MethodCall (target, (Identifier name), paramExps) ->
      let deps, g1, target', isDone = dataflowE env types graph target
      if isDone
  ...
  | ArrayIndex (target, index) ->
      ...
      | _ -> let depsTrg, g', target', isDoneTrg = dataflowE env types graph target
             let depsIdx, g'', index', isDoneIdx = dataflowE env types g' index
  ...
  | MemberAccess (expr, (Identifier name)) ->
      let deps, g', expr', isDone = dataflowE env types graph expr
  ...
  | Record fields ->
      let deps, g', exprs, isDone =
        List.fold_left (fun (depsAcc, g, exprsAcc, allDone) (Symbol field, expr) ->
                          let deps, g', expr', isDone = dataflowE env types g expr
  ...
  | Let (Identifier name, binder, body) ->
      let deps1, g1, binder', isDone1 = dataflowE env types graph binder
      let env' = env.Add(name, NodeInfo.AsUnknown(name))                                      // FIXME: Create a final node for deps1 and use it?
      let deps2, g2, body', isDone2 = dataflowE env' types g1 body
      Set.union deps1 deps2, g2, Let (Identifier name, binder', body'), isDone1 && isDone2
  | If (cond, thn, els) ->
      let deps1, g1, cond', isDone1 = dataflowE env types graph cond
      let deps2, g2, thn', isDone2 = dataflowE env types g1 thn
      let deps3, g3, els', isDone3 = dataflowE env types g2 els
      Set.union (Set.union deps1 deps2) deps3, g3, If (cond', thn', els'), isDone1 && isDone2 && isDone3
  | BinaryExpr (oper, expr1, expr2) as expr ->
      let deps1, g1, expr1', isDone1 = dataflowE env types graph expr1
      let deps2, g2, expr2', isDone2 = dataflowE env types g1 expr2
...
and dataflowFuncCall env types graph fn paramExps expr =
  ...
  | Id (Identifier "when") ->
      ...
      | [target; Lambda ([Identifier arg], handler)] ->
          // Dataflow the target
          let depsTarget, g', target', _ = dataflowE env types graph target
          ...
          let depsHandler, g'', handler', _ = dataflowE env' types' g' handler
  ...
  | _ -> let deps, g', paramExps' = List.fold_left (fun (depsAcc, g, exprs) expr ->
                                                      let deps, g', expr', _ = dataflowE env types g expr
...
and makeSubExprBuilder (arg, argType, argMaker) expr deps =
  ...
  let env, types, graph =
    Set.fold_left (fun (env, types, graph) n ->
                     Map.add n.Uid n env,
                     Map.add n.Uid n.Type types,
                     Graph.add ([], n.Uid, n, []) graph)
                  (Map.add arg argInfo Map.empty,
                   Map.add arg argInfo.Type Map.empty,
  ...
  (fun prio operators -> let fixPrio p = Priority.add (Priority.down prio) p
                         ...
                         let final = makeFinalOper env' types' g' expr' deps' operators' (Priority.down prio) arg
                         operators'.[arg], final)
...
and dataflowGroupby env types graph target paramExps =
  ...
  let valueType = typeOf (types.Add(arg, argType)) expr
...
and dataflowSelect env types graph target paramExps =
  ...
  let deps, g', expr', _ = dataflowE env' types' graph subExpr
  let expr'' = Lambda ([Identifier arg], expr')
