...
let rec dataflow (env:NodeContext, types:TypeContext, (graph:DataflowGraph), roots, k) = function
  | Def (Identifier name, exp) ->
      let deps, g', expr', _, k' = dataflowE env types graph exp k
  ...
  | Expr expr ->
      let deps, g', expr', _, k' = dataflowE env types graph expr k
      env, types, g', roots, k'
...
and dataflowE (env:NodeContext) types (graph:DataflowGraph) expr k =   
  ...
  | MethodCall (target, (Identifier name), paramExps) ->
      let deps, g1, target', isDone, k1 = dataflowE env types graph target k
      if isDone
        then let n, g2 = makeFinalNode env types g1 target' deps ("<X>." + name + "()")
             ...
             deps', g3, expr', true, k2
        else deps, g1, MethodCall (target', (Identifier name), paramExps), false, k1
  | ArrayIndex (target, index) ->
      match index with
      | Time (_, _) -> dataflowE env types graph (MethodCall (target, Identifier "[]", [index])) k
      | _ -> let depsTrg, g', target', isDoneTrg, k' = dataflowE env types graph target k
             let depsIdx, g'', index', isDoneIdx, k'' = dataflowE env types g' index k'
             Set.union depsTrg depsIdx, g'', ArrayIndex (target', index'), isDoneTrg && isDoneIdx, k''
  | FuncCall (fn, paramExps) -> dataflowFuncCall env types graph fn paramExps expr k
  | MemberAccess (expr, (Identifier name)) ->
      let deps, g', expr', isDone, k' = dataflowE env types graph expr k
      deps, g', MemberAccess (expr', (Identifier name)), isDone, k'
  | Record fields ->
      let deps, g', exprs, isDone, k' =
        List.fold_left (fun (depsAcc, g, exprsAcc, allDone, k) (Symbol field, expr) ->
                          let deps, g', expr', isDone, k' = dataflowE env types g expr k
                          Set.union depsAcc deps, g', exprsAcc @ [Symbol field, expr'], allDone && isDone, k')       // Is the order relevant?
                       (Set.empty, graph, [], true, k) fields
      deps, g', Record exprs, isDone, k'
  | Let (Identifier name, binder, body) ->
      let deps1, g1, binder', isDone1, k1 = dataflowE env types graph binder k
      let env' = env.Add(name, NodeInfo.AsUnknown(name))                                      // FIXME: Create a final node for deps1 and use it?
      let deps2, g2, body', isDone2, k2 = dataflowE env' types g1 body k1
      Set.union deps1 deps2, g2, Let (Identifier name, binder', body'), isDone1 && isDone2, k2
  | If (cond, thn, els) ->
      let deps1, g1, cond', isDone1, k1 = dataflowE env types graph cond k
      let deps2, g2, thn', isDone2, k2 = dataflowE env types g1 thn k1
      let deps3, g3, els', isDone3, k3 = dataflowE env types g2 els k2
      Set.union (Set.union deps1 deps2) deps3, g3, If (cond', thn', els'), isDone1 && isDone2 && isDone3, k3
  | BinaryExpr (oper, expr1, expr2) as expr ->
      let deps1, g1, expr1', isDone1, k1 = dataflowE env types graph expr1 k
      let deps2, g2, expr2', isDone2, k2 = dataflowE env types g1 expr2 k1
      Set.union deps1 deps2, g2, BinaryExpr (oper, expr1', expr2'), isDone1 && isDone2, k2
  | Seq (expr1, expr2) ->
      let deps1, g1, expr1', isDone1, k1 = dataflowE env types graph expr1 k
      let deps2, g2, expr2', isDone2, k2 = dataflowE env types g1 expr2 k1
      Set.union deps1 deps2, g2, Seq (expr1', expr2'), isDone1 && isDone2, k2
  | Id (Identifier name) ->
      let info = Map.tryfind name env
      match info with
      | Some info' -> if info'.Type <> TyUnit
                        then Set.singleton info', graph, Id (Identifier info'.Uid), true, k
                        else Set.empty, graph, expr, false, k // If the type is unknown, nothing depends on it.
      | _ -> raise (UnknownId name)
  | Integer i -> Set.empty, graph, expr, true, k
  | String s -> Set.empty, graph, expr, true, k
  | Bool b -> Set.empty, graph, expr, true, k
...
and dataflowFuncCall env types graph fn paramExps expr k =
  ...
  | Id (Identifier "stream") ->
      ...
      Set.singleton n, g', Id (Identifier n.Uid), true, k
  | Id (Identifier "when") ->
      match paramExps with
      | [target; Lambda ([Identifier arg], handler)] ->
          // Dataflow the target
          let depsTarget, g', target', _, k' = dataflowE env types graph target k
          ...
          let depsHandler, g'', handler', _, k'' = dataflowE env' types' g' handler k'
  ...
  | _ -> let deps, g', paramExps', k' = List.fold_left (fun (depsAcc, g, exprs, k) expr ->
                                                          let deps, g', expr', _, k' = dataflowE env types g expr k
                                                          Set.union depsAcc deps, g', exprs @ [expr'], k')
                                                       (Set.empty, graph, List.empty, k) paramExps
         deps, g', FuncCall (fn, paramExps'), true, k'
...
and dataflowDictOps env types graph target paramExps argType argMaker resType opMaker methName k =
  ...
  let deps, g2, body', _, k' = dataflowE env' types' g1 body k
...
and makeSubExprBuilder initial final body deps env types graph =
  fun prio operators ->
    ...
    //let final = makeFinalOper env types graph body deps operators' (Priority.down prio) arg
...
and makeFinalNode env types graph expr deps name =
  ...
  | Record fields ->
      let g'', fieldDeps = List.fold_left (fun (g, fieldDeps) (Symbol field, expr) ->
                                             ...
                                             let deps'', g', expr', isDone, k' = dataflowE env' types' g expr ForwardDeps.empty
...
and dataflowSelect env types graph target paramExps k =
  ...
  let deps, g', expr', _, k' = dataflowE env' types' graph subExpr k
