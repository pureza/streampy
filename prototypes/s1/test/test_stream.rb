...
class StreamTest < Test::Unit::TestCase
    ...
    def add_data
        10.times do |n|
            @stream.add(Tuple.new(n, :set => n % 3, :value => n))
            Clock.instance.advance 1
        end
    end

    def test_map
        other = @stream.map { |tuple| { :value => tuple.value * 2, :set => tuple.set } }
        add_data()
    ...
    def test_groupby
        other = @stream.groupby(:set) { |stream| stream.filter { |t| t.value.odd? }.sum(:value) }
        add_data()

        assert_equal [3, 12], other[0].data.map { |t| t.value }
        assert_equal [1, 8], other[1].data.map { |t| t.value }
        assert_equal [5], other[2].data.map { |t| t.value }
    end


    def test_groupby_window
        other = @stream[(-4.s .. Now)].groupby(:set) { |stream| stream.filter { |t| t.value.odd? }.sum(:value) }
        add_data()

        assert_equal [3, 0, 9], other[0].data.map { |t| t.value }
        assert_equal [3, 8, 9], other[0].data.map { |t| t.timestamp }
        assert_equal [1, 0, 7], other[1].data.map { |t| t.value }
        assert_equal [5, 0], other[2].data.map { |t| t.value }
    end
    ...
    def test_windowed
        other = @stream[(-4.s .. Now)] >> map { |elem| { :value => elem[:value] } }
        add_data()

        assert_equal [6, 7, 8, 9], other.data.map { |t| t.value }

        Clock.instance.advance 1
        assert_equal [7, 8, 9], other.data.map { |t| t.value }
    end
    ...
    def test_complex_query
        avg_value = @stream[(-9.s .. Now)].avg(:value)
        value_equals_avg = @stream.groupby(:set) { |stream| stream.filter { |tuple| tuple.value == avg_value.data } }

        add_data()

        assert_equal [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5], avg_value.data.map { |t| t.value }
